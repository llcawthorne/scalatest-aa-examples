...
/**
  * These are different algorithms to generate a `List[Int]`
  * (technically they generate a `Gen[List[Int]]`.
  */
object GenIntSeq {
  ...
  // (2) generate lists using a frequency, where 2s occur four time as often
  // as the numbers 1, 3, 4, 5
  ...
  // it improves on what's show on this page:
...
object DropAllButFirstSpec_IntLists extends Properties("DropAllButFirstSpec") {
  ...
  property("dropAllButFirstIntLists") = forAll(GenIntSeq.genMostlyTwos) { input: List[Int] =>

    // for debugging
    //println(s"input: $input")

    // shorten list and call `dropAllButFirst`
    val input2 = input.take(20)
    val result = ListUtils.dropAllButFirst(input2, 2)

    // start the tests
    val numMatches = input2.count(_ == NUM_TO_DROP)
    if (numMatches == 0) {
      /**
      ...
      val element1PositionOriginal = input2.indexOf(NUM_TO_DROP)
      ...
      val locationOfFirstOccurrenceInInput = input2.indexOf(NUM_TO_DROP)
      val (inputBefore, inputAfter) = input2.splitAt(locationOfFirstOccurrenceInInput)
